 ...
 * @param {options} object Optional. Options to modify the behavior of this function.
 * @param {options.stringToNumber} boolean. Defaults to `false'. If `true` then it will try to cast the string as a number if the string looks like a valid number.
 * @returns {number} The original number passed to `stringifyId()`.
 * @example
 * parseId('vJH34sj3') // 1
 */
export const parseId = (str, options = {}) => {

    if (isNumber(str)) {
        return str
    }
    else if (options.stringToNumber && !isNaN(str)) {
        return parseInt(str);
    }
    else {
        try {
