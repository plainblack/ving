...
The `code` is always an integer and conforms to the standard list of [Error Codes](/ving/error-codes). These numbers are used consistently so that app developers can trap and handle specific types of exceptions more gracefully.
...
There is a composable built into ving called [useVingRecord](/ving/ui/useVingRecrd) that will allow you to access the Rest API easily for individual records.

### useVingKind Composable
There is a composable built into ving called [useVingKind](/ving/ui/useVingKind) that will allow you to access the Rest API easily for lists of records. It uses the useVingRecord composable underneath to give you access to the individual records within the list.
